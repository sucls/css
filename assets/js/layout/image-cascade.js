 ...
 * @returns 
 */
function createImg(opt){
    // return new DOMParser().parseFromString(`<div class="image-item"><img src="${src}" alt=""></div>`, 'text/html').body.childNodes[0];
    let { src } = opt;
    let div = document.createElement('div');
    div.classList.add('image-item');
    div.innerHTML= `<img src="${src}" alt="">${createImgTooltip()}</div>`; //
    let image = new Image();
    image.src = src;
    image.onload = (evt)=>{ 
...
function createImgViewer(src){
    ...
    return new DOMParser().parseFromString(tpl, 'text/html').body.childNodes[0];
}

function createImgProperty(prop){
    const { src } = prop;
...
function handleImageItemClick (evt){
    ...
    let viewer = createImgViewer(evt.target.src);
